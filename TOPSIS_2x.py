...
def create_topsis_dataset(threshold, dataset, W_val=W_val, criterion_type_val=criterion_type_val):
    ...
    for row in dataset:
        # check if the point is the same as middle point(if yes then topsis will not take it)
        if row[0] == 0.5 and row[1] == 0.5:
            final_scores_matrix = np.append(final_scores_matrix, 2)
            continue
        matrix = np.vstack((row, MIDDLE_POINT))
        closeness_matrix = topsis_method(matrix, weights, criterion_type, graph=False, verbose=False)
    ...
    save_data_to_file(df=df, filename=f"data/TOPSIS_2x/TOP_2x_TH{threshold}.csv")
